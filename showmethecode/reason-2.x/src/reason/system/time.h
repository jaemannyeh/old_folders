...
class Time: public TimeLibrary, public Printable
{
public:
	...
	{
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?local.tm_hour:0;
	}

	int Minute()
	{
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?local.tm_min:0;
	}

	int Second()
	{
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?local.tm_sec:0;
	}
	...
	int Weekday()
	{
		...
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?local.tm_wday:0;
	}
	...
	int Day()
	{
		...
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?local.tm_mday:0;
	}
	...
	static String Day(Time & time)
	{
		...
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&time,&local))?String(local.tm_mday):0;
	}
	...
	int Month()
	{
		...
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?local.tm_mon+1:0;
	}
	...
	int Year()
	{
		...
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?local.tm_year+1900:0;
	}
	...
	static String Year(Time & time)
	{
		...
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&time,&local))?String(local.tm_year+1900):0;
	}

	using TimeLibrary::IsLeapYear;
	bool IsLeapYear() 
	{
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?IsLeapYear(local.tm_year+1900):false;
	}

	using TimeLibrary::DaysInYear;
	int DaysInYear()
	{
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?DaysInYear(local.tm_year+1900):0;
	}

	using TimeLibrary::DaysInMonth;
	int DaysInMonth()
	{
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)&Seconds,&local))?DaysInMonth(local.tm_mon+1):0;
	}
	...
	void Print(String & string)
	{
		...
		Formatter::Append(string,"%s %s %02d %04d",Weekday(*this).Substring(0,3).Print(), Month(*this).Substring(0,3).Print(),this->Day(),this->Year()); 
		string << " ";
...
public:
	...
	{
		struct tm local;
		return (TimeLibrary::Gmtime((time_t*)this,&local))?local.tm_wday:0;
	}

	void Print(String & string)
	{
		struct tm local;
		if (TimeLibrary::Gmtime((time_t*)this,&local)) string.Append(Weekday::Weekdays[local.tm_wday]);
	}
