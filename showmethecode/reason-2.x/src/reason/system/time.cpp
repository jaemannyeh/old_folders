...
bool Time::Construct()
{
	...
	struct tm local;
	if (TimeLibrary::Localtime((time_t*)&time.time,&local))
	{
		local.tm_isdst=0;
...
String Time::Weekday(Time & time)
{
	...
	struct tm local;
	return (TimeLibrary::Gmtime((time_t*)&time.Seconds,&local))?String(Weekday::Weekdays[local.tm_wday]):String();
}
...
String Time::Month(Time & time)
{
	...
	struct tm local;
	return (TimeLibrary::Gmtime((time_t*)&time.Seconds,&local))?String(Month::Months[local.tm_mon]):String();
}
	...
	if (IsAny(1,"/-.") || IsAny(2,"/-.") || IsAny(4,"/-."))
	{
		...
		if (Token.Size <= 2)
		{
			...
			if (ParseDateLiteral() && Token.Size <= 2 && IsAny("/-."))
			{
				month = Token.Integer();
				Next();
				if (ParseDateLiteral() && Token.Size == 4 && (IsWhitespace() || IsSymbol() || Eof()))
				{	
					year = Token.Integer();
...
bool TimeParser::ParseDateLiteral()
{
	...
	{
		ParseNumber();
		if ((Token.Size <= 2 || Token.Size == 4) && (IsAny("/-.") || IsWhitespace() || IsSymbol() || Eof()))
		{
...
bool Calendar::Construct(const time_t & seconds)
{
	...
	struct tm local;
	if (TimeLibrary::Gmtime(&seconds,&local))
	{
...
void Calendar::Format(String & string, const char * format)
{
	...
	if (Year && Month)
	{
		...
		time_t seconds = mktime(&local);
		if (TimeLibrary::Gmtime(&seconds,&local)) string.Size = strftime(string.Data,string.Allocated-1,format,&local);
	}
...
long Timer::ElapsedMilliseconds()
{
	...
	return (double)ElapsedNanoseconds() / Time::NanosecondsPerMillisecond;
}
...
long long Timer::ElapsedNanoseconds()
{
	...
	if (IsStopped())
	{
		return  (double)(Stopped-Started) / ((double)Frequency / Time::NanosecondsPerSecond);
	}
	else
	if (IsStarted())
	{
		...
		return  (double)(now-Started) / ((double)Frequency / Time::NanosecondsPerSecond) ;
	}
	else
	{
		return 0;
	...
	if (IsStopped())
	{
		long long secondsToNanoseconds = (Stopped.tv_sec - Started.tv_sec) * Time::NanosecondsPerSecond;
		#ifdef REASON_PLATFORM_APPLE
		return secondsToNanoseconds + (Stopped.tv_usec - Started.tv_usec);
		#else	
	...
	if (IsStarted())
	{
		...
		long long secondsToNanoseconds = (now.tv_sec - Started.tv_sec) * Time::NanosecondsPerSecond;
		#ifdef REASON_PLATFORM_APPLE
		return  secondsToNanoseconds + (now.tv_usec - Started.tv_usec);
		#else
...
double Timer::ElapsedSeconds()
{
	...
	return (double)ElapsedNanoseconds() / Time::NanosecondsPerSecond;
}
