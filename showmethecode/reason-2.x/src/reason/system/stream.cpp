...
int Reader::Read(String &string, int amount)
{
	...
	else
	{
		...
		int offset = 0;
		string.Size = 0;
		while(amount > 0 && (amount=Read(string.Data+offset,amount)))
		{
			string.Size += amount;
...
int Writer::Write(Reader & reader, int amount)
{
	...
	String writer;
	writer.Resize((amount>0)?amount:4096);
	int read=0;
	int write=0;
...
StreamTokenizer::StreamTokenizer(Reason::System::Stream & stream):Stream(stream)
{
}
...
bool StreamTokenizer::Move()
{
	return Move(1);
}

bool StreamTokenizer::Move(int amount)
{
...
bool StreamTokenizer::Reverse()
{
	OutputError("StreamEnumerator::Last - Method not implemented.\n");
	return true;
}

bool StreamTokenizer::Has()
{
	return !Token.IsNull();
}

Substring StreamTokenizer::Next()
{
    ...
    Buffer.Contract(-(Token.Size+Separator.Size));
	...
	while(match.IsNull() && (Stream.IsReadable() || Buffer.Size > 0))
	{
		...
		for(separator.Forward();separator.Has();separator.Move())
		{
			...
			if ( (index >= 0) && (index < found || found < 0)  )
			{
				...
				match = Buffer.Substring(0,index);					
				found = index;
...
Substring StreamTokenizer::Prev()
{
...
int StreamBuffer::Seek(int position, int origin)				
{
	...
	if (origin < 0)
	{
		...
		if (Offset+relative >= Buffer.Data)
		{
