...
public:
	...
	class SocketLibrarySentinel
	{
	public:
		SocketLibrarySentinel()
		{
	...
	static const SocketLibrarySentinel Library;
...
public:
	...
	bool IsAsynchronous()	{return Mode == SocketLibrary::SOCKET_MODE_ASYNCHRONOUS;}
	bool IsSynchronous()	{return Mode == SocketLibrary::SOCKET_MODE_SYNCHRONOUS;}

	bool IsReadable(unsigned int timeout);
	bool IsWriteable(unsigned int timeout);

	bool IsReadable()		{return IsReadable(SocketLibrary::TimeoutImmediate);}
	bool IsWriteable()		{return IsWriteable(SocketLibrary::TimeoutImmediate);}

	bool IsErroneous()		{return State.Is(SocketLibrary::SOCKET_STATE_ERROR);}
	bool IsTimeout()		{return State.Is(SocketLibrary::SOCKET_STATE_TIMEOUT);}

	bool IsCreated()		{return Descriptor != SocketLibrary::SocketInvalid;}
	bool IsDisconnected()	{return !IsConnected();}
	bool IsBound()			{return State.Is(SocketLibrary::SOCKET_STATE_BOUND);}
	bool IsListening()		{return State.Is(SocketLibrary::SOCKET_STATE_LISTENING);}

	bool IsConnected(unsigned int timeout);
	bool IsConnected()		{return IsConnected(SocketLibrary::TimeoutImmediate);}

	virtual bool Create(int family = SocketLibrary::SOCKET_FAMILY_INET, int type = SocketLibrary::SOCKET_TYPE_STREAM, int protocol = SocketLibrary::SOCKET_PROTOCOL_TCP);
	...
	virtual bool Select(char selector, unsigned int timeout);
	virtual bool Select(char selector) {return Select(selector,SocketLibrary::TimeoutImmediate);};

	virtual int Send(char * data, int size, unsigned int timeout=SocketLibrary::TimeoutDefault);
	virtual int Receive(char * data, int size, unsigned int timeout=SocketLibrary::TimeoutDefault);

	int Error();

	int Read(char * data, int size, unsigned int timeout=SocketLibrary::TimeoutDefault);
	int Write(char* data, int size, unsigned int timeout=SocketLibrary::TimeoutDefault);
	...
	void Option(int level, int option, bool value) {Option(level,option,(char*)&value,sizeof(bool));}
	void Option(int level, int option, int value) {Option(level,option,(char*)&value,sizeof(int));}
	void Option(int level, int option, char * data, const int size);

protected:

	void Readable(bool read);
	void Writeable(bool write);
	void Erroneous(bool error);
...
public:
	...
	Connection(int mode):Socket(mode) {}
...
public:
	...
	int Send(   Address & destinationAddress,   const char * data,   const int size);
	int Receive(   Address & sourceAddress,   char * data,   const int size);
...
public:
	...
	virtual bool Create(int type = SocketLibrary::SOCKET_TYPE_STREAM, int protocol = SocketLibrary::SOCKET_PROTOCOL_TCP);
	virtual bool Destroy();
