...
void HttpRequest::Initialise()
{
	...
	Timeout = SocketLibrary::TimeoutDefault;
}
...
bool HttpRequest::Construct(const Reason::Network::Url &url, int options)
{
	...
	else
	{
		...
		if (url.Path->StartsWith("/"))
			Uri->Replace(((Url&)url).Substring(url.Path));
		else
			Uri->Replace("/" + ((Url&)url).Substring(url.Path));
	}
...
HttpManager::HttpManager():ConnectionCount(0),ConnectionLimit(15),ListenSocket(SocketLibrary::SOCKET_MODE_ASYNCHRONOUS)
{
}
...
unsigned int HttpManager::Run(Thread::Pointer * pointer)
{
...
int HttpPublisher::Process(HttpContext & context, HttpRequest & request, HttpResponse & response)
{
	...
	if (url.IsRelative())
	{
		...
		if ( secure && ! url.Path->IsEmpty() )
		{
			...
			relative = filesystem.Substring(filesystem.IndexOf("web")+3);
		}
		else
		{
			relative = url;
				...
				else
				{
					...
					for(folder.Files.Forward();folder.Files.Has();folder.Files.Move())
					{
						if (!relative.IsEmpty())
						{
							...
							listing << "<td class=name><a href=\"";
							listing << relative << "/" << folder.Files()->Fullname();
							listing << "\">" << folder.Files()->Fullname() << "</a></td>";									
							listing << "</tr>";
						}
					}
...
HttpServer::HttpServer():ListenSocket(SocketLibrary::SOCKET_MODE_ASYNCHRONOUS)
{
	ConnectionLimit = 15;
	ConnectionCount = 0;
}
	...
	if ( ! request.IsEmpty() )
	{
		...
		if (request.Deconstruct())
		{
			if (!request.ContentLength->IsEmpty())
			{
				...
				request.Body->Assign(request.SubstringAfter(separator,4));
			}
...
{
	Mutex::Auto lock(Mutex);
	return List.Count > 0 && (Sleeping < List.Count || Has());
}
...
void HttpAgent::Run(void *pointer)
{
	int id = ((Thread::Pointer*) pointer)->Id;
...
void HttpAgent::Process(Link & link)
{
	...
	referer.Append((const Sequence&)link.Substring(0,link.LastIndexOf('/')+1));
