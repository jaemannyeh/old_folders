	...
	HttpConnection * Open(const Address & address)
	{
		...
		for (iterator.Forward();iterator.Has();iterator.Move())
		{
			if (iterator().Value())
			{
				...
				if (iterator().Key()->Socket.IsErroneous() || iterator().Key()->Socket.IsTimeout())
				{
...
class HttpManager: public Thread, public HttpConstants
{
protected:

	Reason::Platform::Mutex Sentinel;
...
public:
	...
	Structure::Enumeration<HttpService*> Services;
